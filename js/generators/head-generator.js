...
async function updateHead(attributes, setup) {
  ...
  if (setup.general?.include_e_commerce && setup.general?.snipcart) {
    ...
    // 1. Build the settings object that the docs expect
    const settings = {
      ...
      // version is optional – omit when empty → latest
      ...(snip.version && snip.version.trim() ? { version: snip.version.trim() } : {}),
      // keep any extra flags you might add later
      ...(snip.loadCSS === false ? { loadCSS: false } : {})
    };

    // 2. Create the inline script that the docs ship
    const script = document.createElement('script');
    script.textContent = `
  ...
  `;
    ...
    // 3. Insert **after** the opening <body> (best practice per docs)
    //    We append it to the deferred fragment – it will land in <head> but
    //    the loader itself moves the hidden div into <body>.
    deferredFrag.appendChild(script);

    logger.log('Snipcart bootstrapper injected', settings);
  }
