...
// Helper function to check if a video source has a valid extension.
...
// Main exported function to generate <video> markup.
// Supports light/dark theme variants, posters, custom classes, autoplay, mute, loop, and more.
// Uses caching for performance; handles generation on the main thread for simplicity and reduced overhead.
export function generateVideoMarkup({
  ...
  try {
    ...
    if (isDev) console.log('Generated video markup:', markup.substring(0, 200));
    return markup;
  } catch (error) {
    if (isDev) console.error('Error generating video markup:', error);
    const primarySrc = lightSrc || darkSrc || src;
    return `<video><p>Error generating video: ${error.message}</p><a href="${primarySrc || '#'}">Download video</a></video>`;
  }
}
