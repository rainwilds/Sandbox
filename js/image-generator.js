...
export function generatePictureMarkup({
  ...
  const cacheKey = JSON.stringify({
    ...
    fullSrc, fullLightSrc, fullDarkSrc, fullAlt, fullLightAlt, fullDarkAlt,
  ...
  if (markupCache.has(cacheKey)) {
    if (isDev) console.log('Using cached picture markup:', cacheKey);
  ...
  src = trim(src);
  lightSrc = trim(lightSrc);
  darkSrc = trim(darkSrc);
  fullSrc = trim(fullSrc);
  fullLightSrc = trim(fullLightSrc);
  fullDarkSrc = trim(fullDarkSrc);
  iconSrc = trim(iconSrc);
  iconLightSrc = trim(iconLightSrc);
  iconDarkSrc = trim(iconDarkSrc);
  alt = trim(alt);
  lightAlt = trim(lightAlt);
  darkAlt = trim(darkAlt);
  fullAlt = trim(fullAlt);
  fullLightAlt = trim(fullLightAlt);
  fullDarkAlt = trim(fullDarkAlt);
  iconAlt = trim(iconAlt);
  iconLightAlt = trim(iconLightAlt);
  iconDarkAlt = trim(iconDarkAlt);
  customClasses = trim(customClasses);
  extraStyles = trim(extraStyles);

  // Pre-validate image sources with HEAD requests
  ...
  const sourcesToValidate = [
    src, lightSrc, darkSrc, fullSrc, fullLightSrc, fullDarkSrc,
    iconSrc, iconLightSrc, iconDarkSrc
  ].filter(Boolean);
  ...
  if (hasInvalidSource) {
    ...
    const errorMsg = `One or more image sources not found: ${invalidUrls.join(', ')}`;
    if (isDev) console.error('Image source validation failed:', errorMsg);
  ...
  const workerCode = `
    ...
    function parseWidth(widthStr, winWidth = 1920) {
