...
import {
  VALID_ALIGNMENTS, alignMap, VALID_FETCH_PRIORITIES, VALID_HEADING_TAGS,
  VALID_POSITIONS, VALID_BUTTON_TYPES, VALID_BUTTON_RELS, VALID_VIDEO_EXTENSIONS,
  IMAGE_WIDTHS, IMAGE_FORMATS, SIZES_BREAKPOINTS, DEFAULT_SIZE_VALUE, BASE_PATH,
  BACKDROP_FILTER_MAP, sanitizeClassNames, sanitizeStyles, validateCssLength,
  validateFontAwesomeIcon, withLazyLoading, withRenderCaching
} from '../shared.js';

class CustomBlockBase extends HTMLElement {
  constructor() {
    super();
    this.callbacks = [];
  ...
  getAttributes() {
    ...
    if (!VALID_FETCH_PRIORITIES.includes(backgroundFetchPriority)) {
      console.warn(`Invalid img-background-fetchpriority value "${backgroundFetchPriority}" in <custom-block>. Using default.`);
    }
    if (!VALID_FETCH_PRIORITIES.includes(primaryFetchPriority)) {
    ...
    if (!VALID_POSITIONS.includes(primaryPosition)) {
    ...
    if (!VALID_HEADING_TAGS.includes(headingTag.toLowerCase())) {
      console.warn(`Invalid heading-tag value "${headingTag}" in <custom-block>. Must be one of ${VALID_HEADING_TAGS.join(', ')}. Using default 'h2'.`);
    }
    if (!VALID_HEADING_TAGS.includes(subHeadingTag.toLowerCase())) {
      console.warn(`Invalid sub-heading-tag value "${subHeadingTag}" in <custom-block>. Must be one of ${VALID_HEADING_TAGS.join(', ')}. Using default 'h3'.`);
    }
    ...
    if (icon) {
      icon = validateFontAwesomeIcon(icon);
    }
    ...
    if (iconStyle) {
      ...
      sanitizedIconStyle = sanitizeStyles(iconStyle, allowedStyles);
    ...
    let sanitizedIconClass = sanitizeClassNames(iconClass);
    ...
    let sanitizedIconSize = validateCssLength(iconSize, ['rem']);
    if (sanitizedIconSize !== iconSize) {
    ...
    let sanitizedButtonClass = sanitizeClassNames(buttonClass);
    ...
    if (buttonStyle) {
      ...
      sanitizedButtonStyle = sanitizeStyles(buttonStyle, allowedStyles);
    ...
    if (buttonRel) {
      sanitizedButtonRel = buttonRel.split(/\s+/).filter(rel => VALID_BUTTON_RELS.includes(rel)).join(' ');
      if (sanitizedButtonRel !== buttonRel) {
        console.warn(`Invalid button-rel value "${buttonRel}" in <custom-block>. Must be one of ${VALID_BUTTON_RELS.join(', ')}. Using sanitized: "${sanitizedButtonRel}".`);
      }
    }
    ...
    if (buttonType && VALID_BUTTON_TYPES.includes(buttonType)) {
      ...
      console.warn(`Invalid button-type value "${buttonType}" in <custom-block>. Must be one of ${VALID_BUTTON_TYPES.join(', ')}. Using default ${buttonHref ? "'link'" : "'button'"}.`);
    }
    let buttonIcon = this.getAttribute('button-icon') || '';
    if (buttonIcon) {
      buttonIcon = validateFontAwesomeIcon(buttonIcon);
    }
    ...
    let sanitizedButtonIconSize = validateCssLength(buttonIconSize, ['rem']);
    if (sanitizedButtonIconSize !== buttonIconSize) {
    ...
    let sanitizedEffects = sanitizeClassNames(effects);
    ...
    this.cachedAttributes = {
      ...
      backgroundFetchPriority: VALID_FETCH_PRIORITIES.includes(backgroundFetchPriority) ? backgroundFetchPriority : '',
      ...
      primaryFetchPriority: VALID_FETCH_PRIORITIES.includes(primaryFetchPriority) ? primaryFetchPriority : '',
  ...
  render(isFallback = false) {
    ...
    const hasPrimaryImage = !isFallback && !!(attrs.primarySrc || attrs.primaryLightSrc || attrs.primaryDarkSrc) && VALID_POSITIONS.includes(attrs.primaryPosition);
    const hasVideoPrimary = !isFallback && !!(attrs.videoPrimarySrc || attrs.videoPrimaryLightSrc || attrs.videoPrimaryDarkSrc) && VALID_POSITIONS.includes(attrs.primaryPosition);
    ...
    if (hasVideoBackground) {
      let videoMarkup = generateVideoMarkup({
    ...
    const iconStyles = attrs.iconStyle || (attrs.iconSize ? `font-size: ${attrs.iconSize}` : '');
  ...
  static get observedAttributes() {
    return [
      ...
      'button-rel', 'button-style', 'button-target', 'button-text', 'button-type', 'class',
      'effects', 'heading', 'heading-tag', 'icon', 'icon-class', 'icon-size', 'icon-style',
      ...
      'img-primary-tablet-width', 'inner-alignment', 'inner-backdrop-filter', 'inner-background-color',
      'inner-background-gradient', 'inner-background-image-noise', 'inner-background-overlay',
      'inner-border', 'inner-border-radius', 'inner-class', 'inner-shadow', 'inner-style',
      'section-title', 'shadow', 'style', 'sub-heading', 'sub-heading-tag', 'text',
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    const criticalAttributes = [
      ...
      'inner-shadow', 'inner-style', 'section-title', 'style', 'sub-heading',
...
export const CustomBlock = withRenderCaching(withLazyLoading(CustomBlockBase));
