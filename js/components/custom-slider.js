    ...
    #validateIcon(icon, attributeName) {
        ...
        if (!iElement) {
            this.#warn(Invalid ${ attributeName } format: No < i > tag found, {
                ...
                expected: 'Font Awesome  tag with fa- classes'
            });
            return '';
        }
        ...
        // Test if Font Awesome is loaded by rendering a temporary icon
        ...
        if (hasNavigation) {
            ...
            if (navigationIconLeft) {
                ...
                if (validatedIconLeft) {
                    ...
                    this.#log('Using fallback icon for navigation-icon-left', { icon: '', elementId: this.id || 'no-id' });
                }
            } else {
                prev.innerHTML = '';
                this.#log('No navigation-icon-left provided, using fallback', { icon: '', elementId: this.id || 'no-id' });
            }
            if (navigationIconRight) {
                ...
                if (validatedIconRight) {
                    ...
                    this.#log('Using fallback icon for navigation-icon-right', { icon: '', elementId: this.id || 'no-id' });
                }
            } else {
                next.innerHTML = '';
                this.#log('No navigation-icon-right provided, using fallback', { icon: '', elementId: this.id || 'no-id' });
            }
